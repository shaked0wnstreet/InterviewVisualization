...
function App() {
  ...
  const [jsonArray, setJsonArray] = useState({{
    ...
    "nodes": [
        ...
        {
            "id": "UserResponseTooLong",
            "DialogText": "I'm sorry but we need to move on"
        }
    ]
}});
  ...
  function start(){
    if (jsonArray) {
      // call the  API to return the json Array 
    }
    else { // if the jsonArray is empty, you call init to intialize the first node
 ...
 const [interviewerDialogs, setInterviewerDialogs] = useState(jsonArray['nodes']);
  ...
  function onAddSubmit(nodeID, newNode) {
    // call the add API: insert node

  }
  function onEditSubmit(nodeID, newNode) {
    // call the edit API edit node
  }
